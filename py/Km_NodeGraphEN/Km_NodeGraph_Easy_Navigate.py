    ...
    def __init__(self, bookmarkData,parrent,width=70,height=70):
        ...
        if self.nodeName != "empty" :
            ...
            if int(nuke.env ["NukeVersionMajor"]) < 13 :
                Km_NGJ.addCommand("  Easy Navigate/Bookmarks/"+str(bookmarkData["title"].encode('utf-8')),lambda: self.JumpToNode(bookmarkData['nodeName'].encode('utf-8')),bookmarkData["shortcut"])
            else :
                Km_NGJ.addCommand("  Easy Navigate/Bookmarks/"+str(bookmarkData["title"]),lambda: self.JumpToNode(bookmarkData['nodeName']),bookmarkData["shortcut"])
...
class SettingsWindow(QMainWindow,Ui_SettingsWindowUI):
    ...
    def applySettings(self): 
        ...
        menu = nuke.menu("Nuke")
        Km_NGJ = menu.addMenu("TPL")
        Km_NGJ.addCommand("  Easy Navigate/Show Panel","Km_NodeGraph_Easy_Navigate.ShowMainWindow()",self.settings["shortcut"])
        self.close()
...
def updateShortcuts():
    ...
    menu = nuke.menu("Nuke")
    if menu.findItem('TPL/  Easy Navigate') :
        tappmenu = menu.findItem('TPL/  Easy Navigate')
