...
class Core_SetLabel(QtWidgets.QDialog):
    def __init__(self):
        ...
        self.help = QtWidgets.QLabel(
            '<span style=" font-size:7pt; color:green;">Ctrl+Enter to confirm</span>'
        )
    ...
    def run(self):
        ...
        if self.getNodes(dag):
            ...
            posx = min(
                max(cursor.x() - 100, avail_space.left()), avail_space.right() - 200
            )
            posy = min(
                max(cursor.y() - 12, avail_space.top()), avail_space.bottom() - 200
            )
